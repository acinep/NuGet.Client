...
namespace NuGetConsole.Host.PowerShell.Test
{
    ...
    {
        [Fact]
        public async Task EnumeratePackagesAsync_ForBuildIntegratedProject()
        {
            ...
            using (var testDirectory = TestDirectory.Create())
            {
                ...
                var packagesFolderPath = PackagesFolderPathUtility.GetPackagesFolderPath(solutionDirectory, testSettings);
        ...
        [Fact]
        public async Task EnumeratePackagesAsync_ForPackagesConfig()
        {
            var sourceRepositoryProvider = TestSourceRepositoryUtility.CreateV3OnlySourceRepositoryProvider();
            ...
            using (var testDirectory = TestDirectory.Create())
            {
                ...
                var target = new InstalledPackageEnumerator(
                    testSolutionManager,
                    testSettings,
                    getLockFileOrNullAsync: _ => Task.FromResult(new LockFile()));
        ...
        private static ISettings PopulateSettingsWithSources(ISourceRepositoryProvider sourceRepositoryProvider, TestDirectory settingsDirectory)
        {
